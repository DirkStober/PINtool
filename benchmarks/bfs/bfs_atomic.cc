...
// Use extern C otherwise compiler will mangle the symbol NAME!
// Use no inline! 
extern "C"{
	
volatile __attribute__((noinline)) void 
	_NDP_PIN_START_STOP_(int i){
	if(i){
	...
	{   
		for(v=start;v<stop;v++)
		{
			...
			{   
				...
				if(Q[neighbor]==1)                                  
				{
					if(__sync_bool_compare_and_swap(&Q[neighbor],1,0))
					{
...
int Usage(){
	printf("No input: ./bfs 0 <Max Input Threads> <N> <DEG>\n");
	printf("Input file: ./bfs 1 <Max Input Threads> <FileName>\n");
	return 0;	
};
...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i < N; i++)
	{
		W[i] = (int *)malloc(sizeof(int)*DEG);
		W_index[i] = (int *)malloc(sizeof(int)*DEG);
	}
	...
	// This is the part to be benchmarked
	// This function tells the PIN tool to start counting misses and hits and so on
	//CPU Time
	printf("Starting timed kernel!\n");
